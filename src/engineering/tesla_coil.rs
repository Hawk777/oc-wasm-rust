...
///
/// This type combines a Tesla coil address, an [`Invoker`](Invoker) that can be used to make
/// method calls, and a scratch buffer used to perform CBOR encoding and decoding. A value of this
/// type can be created by calling [`TeslaCoil::lock`](TeslaCoil::lock), and it can be dropped to
...
impl<'invoker, 'buffer, B: Buffer> Locked<'invoker, 'buffer, B> {
	...
	pub async fn set_redstone_polarity(&mut self, level: SignalLevel) -> Result<(), Error> {
		component_method(
			...
			&self.address,
			"setRSMode",
			Some(&OneValue(level == SignalLevel::Low)),
		)
		.await?;
		Ok(())
	}
	...
	pub async fn set_power(&mut self, level: PowerLevel) -> Result<(), Error> {
		let ret: Result<(), MethodCallError<'_>> = component_method(
			...
			&self.address,
			"setPowerMode",
			Some(&OneValue(level == PowerLevel::High)),
		)
		.await;
		match ret {
			Ok(()) => Ok(()),
