...
impl<'invoker, 'buffer> Locked<'invoker, 'buffer> {
	...
	async fn call_open(&mut self, path: &str, mode: &str) -> Result<descriptor::Owned, Error> {
		let descriptor: Result<OneValue<Option<descriptor::Decoded>>, oc_wasm_safe::error::Error> =
			component_method(
				self.invoker,
				self.buffer,
				&self.address,
