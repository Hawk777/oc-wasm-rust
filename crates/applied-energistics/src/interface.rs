...
///
/// This type combines an ME Interface block address, an [`Invoker`] that can be used to make
/// method calls, and a scratch buffer used to perform CBOR encoding and decoding. A value of this
/// type can be created by calling [`Block::lock`], and it can be dropped to return the borrow of
/// the invoker and buffer to the caller so they can be reused for other purposes.
///
...
///
/// This type combines an ME Interface part address, an [`Invoker`] that can be used to make method
/// calls, and a scratch buffer used to perform CBOR encoding and decoding. A value of this type
/// can be created by calling [`Part::lock`], and it can be dropped to return the borrow of the
