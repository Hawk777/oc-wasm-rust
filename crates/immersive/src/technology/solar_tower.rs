//! Provides high-level access to the solar tower APIs.
...
/// The type name for solar towers.
pub const TYPE: &str = "it_solar_tower";

/// A solar tower component.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct SolarTower(Address);

impl SolarTower {
	/// Creates a wrapper around a solar tower.
	///
	/// The `address` parameter is the address of the solar tower. It is not checked for
	...
	/// Returns the address of the solar tower.
	#[must_use = "This function is only useful for its return value"]
...
impl<'invoker, 'buffer, B: 'buffer + Buffer> Lockable<'invoker, 'buffer, B> for SolarTower {
...
/// A solar tower component on which methods can be invoked.
///
/// This type combines a solar tower address, an [`Invoker`] that can be used to make method calls,
/// and a scratch buffer used to perform CBOR encoding and decoding. A value of this type can be
/// created by calling [`SolarTower::lock`], and it can be dropped to return the borrow of the
...
impl<'invoker, 'buffer, B: Buffer> Locked<'invoker, 'buffer, B> {
	/// Returns the equivalent number of full reflectors pointing at the tower.
	///
	/// Each full unit is equivalent to a fully operational, completely uncovered reflector.
	///
	/// # Errors
	...
	/// Enables or disables computer control of the solar tower.
	///
	/// If `enable` is `true`, the solar tower runs or stops based on the most recent call to
	/// [`set_enabled`](#set_enabled). If `enable` is `false`, the solar tower runs or stops based
	/// on the redstone signal at the control port.
	///
	/// # Errors
	/// * [`BadComponent`](Error::BadComponent)
	/// * [`TooManyDescriptors`](Error::TooManyDescriptors)
	...
	/// Enables or disables the solar tower.
	///
	/// This can only be called if the solar tower is under computer control via a preceding call
	/// to [`enable_computer_control`](#enable_computer_control).
	///
	/// # Errors
	/// * [`BadComponent`](Error::BadComponent)
